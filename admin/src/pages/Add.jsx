  ...
  const onSubmitHandler = useCallback(
    async (e) => {
      ...
      try {
        ...
        toast.error(error.response?.message || 'Something went Wrong while addding product')
      }
    },
    [name, description, popular, price, category, colors, images, token]
  )
      ...
      <form onSubmit={onSubmitHandler} className=' flex flex-col gap-y-3 medium-14 lg:w-[777px]'>
        <div className=' w-full'>
          ...
          <input
            ...
            value={name}
            type="text" placeholder='Write Here..' className=' px-3 py-1.5 ring-1 ring-slate-900/10 rounded bg-white mt-1 max-w-lg' />
        </div>
        <div className=' w-full'>
          ...
          <textarea
            ...
            value={description}
            rows={5} placeholder='Write Here..' className=' px-3 py-1.5 ring-1 ring-slate-900/10 rounded bg-white mt-1 max-w-lg' />
        </div>
        ...
        <div className='flex flex-col sm:flex-row gap-4'>
          ...
          <div>
            ...
            <input
              ...
              value={price}
              type="number" placeholder='10' className=' px-3 
        ...
        <div>
          ...
          <div className="flex gap-2 my-4 ">
            {['Black', 'Red', 'White', 'Blue'].map((color, i) => (
              ...
              flexCenter' style={{ backgroundColor: color.toLocaleLowerCase() }}>
        ...
        <div className=' flex gap-2 pt-2'>
          {['image1', 'image2', 'image3', 'image4'].map((imgKey, i) => (
            <label key={i} htmlFor={imgKey}>
              <img src={images[imgKey] ? URL.createObjectURL(images[imgKey]) : upload_icon} alt="" className=' w-16 h-16
            aspect-square object-cover ring-1 ring-slate-900/5 rounded-lg' />
              <input onChange={(e) => handleImageChange(e, imgKey)}
                type="file" id={imgKey} hidden />
            </label>
          ))}
        </div>
